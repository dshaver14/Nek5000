...
c-----------------------------------------------------------------------
      ...
      if(ifld_k.gt.ifld_omg) ifevalsrc = .true.
...
c-----------------------------------------------------------------------
      subroutine rans_init!(ifld_k_in,ifld_omg_in,ifcoeffs
c    $                       ,coeffs_in,wall_id,ywd_in,model_id)
c
...
c     real coeffs_in(*),ywd_in(*)
     ...
     &         "ERROR: RANS NOT YET SUPPORTED WITH LOW MACH FORMULATION"
      ...
      if(rans_id.eq.0) ifrans_komg_stndrd          = .TRUE.
      if(rans_id.eq.1) ifrans_komg_lowRe           = .TRUE.
      if(rans_id.eq.2) ifrans_komgSST_stndrd       = .TRUE.
      if(rans_id.eq.3) ifrans_komg_stndrd_noreg    = .TRUE.
      if(rans_id.eq.4) ifrans_ktau_stndrd          = .TRUE.
...
c       call copy   (g,   Om_mag2(1,e),       lxyz)
        ...
        call gradm11(omp_x,omp_y,omp_z,t(1,1,1,1,ifld_omg-1),e)

        do i=1,lxyz

          rho   = vtrans(i,1,1,e,1)
          omp   = t(i,1,1,e,ifld_omg-1) ! Current k & omega values
     ...
     $    o_z(i)= omp_z(i)+expn * dfdz_omegb(i,1,1,e) *f_omegb(i,1,1,e)
          omwom(i) =  1.0/(1.0+t(i,1,1,e,ifld_omg-1)/f_omegb(i,1,1,e))
...
c       call copy   (g,   Om_mag2(1,e),       lxyz)
        ...
        do i=1,lxyz
          ...
          mu    = mul(i,1,1,e)
          nu    = mu/rho
          omp   = t(i,1,1,e,ifld_omg-1) ! Current k & omega prime values
          k     = t(i,1,1,e,ifld_k  -1)   ! from previous timestep
          omw   = f_omegb(i,1,1,e)        ! omega wall
          omega = omp + omw               ! total omega
...
c       call copy   (g,   Om_mag2(1,e),       lxyz)
        ...
        do i=1,lxyz
          ...
          mu    = mul(i,1,1,e) 
          nu    = mu/rho
          omega = t(i,1,1,e,ifld_omg-1) ! Current k & omega values
          k     = t(i,1,1,e,ifld_k-1)     ! from previous timestep
...
c================================
      ...
      call comp_StOm (St_mag2, Om_mag2, OiOjSk, DivQ)
      call sqrt_tau(tausq,t(1,1,1,1,ifld_omg-1),ntot)
      call rzero(div,lxyz)
...
c       call copy   (g,   Om_mag2(1,e),       lxyz)
        ...
        call gradm11(k_x,  k_y,  k_z,  t(1,1,1,1,ifld_k    -1),e)
        call gradm11(tau_x,tau_y,tau_z,t(1,1,1,1,ifld_omg-1),e)
        ...
        do i=1,lxyz
          ...
          rho = vtrans(i,1,1,e,1)
          mu  = mul(i,1,1,e)
          tau = t(i,1,1,e,ifld_omg-1) ! Current k & tau    values
          k   = t(i,1,1,e,ifld_k-1)     ! from previous timestep
...
c       call copy   (g,   Om_mag2(1,e),       lxyz)
        ...
        do i=1,lxyz
          ...
          rho   = vtrans(i,1,1,e,1)
          omega = t(i,1,1,e,ifld_omg-1) + f_omegb(i,1,1,e) ! Current k & omega values
          k     = t(i,1,1,e,ifld_k  -1)   ! from previous timestep
...
c limits for k, omega
      ...
      do i=1,nxyz
          ...
          if(t(i,1,1,e,ifld_omg-1).lt.0.0) then
c           write(*,*) 'Zero OMEG ', t(i,1,1,e,ifld_omg-1)
            xome_neg = min(xome_neg,t(i,1,1,e,ifld_omg-1))
            nome_neg = nome_neg + 1
c           write(*,*) 'Neg  OMEG ', nome_neg, xome_neg
            t(i,1,1,e,ifld_omg-1)=frac*abs(t(i,1,1,e,ifld_omg-1))
          endif
...
c limits for k, tau
          ...
          if(t(i,1,1,e,ifld_omg-1).lt.0.0) then
c           write(*,*) 'Zero TAU  ', t(i,1,1,e,ifld_omg-1)
            xtau_neg = min(xtau_neg,t(i,1,1,e,ifld_omg-1))
            ntau_neg = ntau_neg + 1
